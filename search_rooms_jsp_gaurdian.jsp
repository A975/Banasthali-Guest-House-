        ...
        ResultSet rs=stat.executeQuery(qry); 
          ...
          <%
           ...
           while(rs.next())
           {
             ...
             <tr><td><%=rs.getInt(1)%></td><td><%=rs.getString(2) %></td><td><%=rs.getInt(3)%></td><td><a href="display_loggedIn_User_guardian.jsp?room_no=<%=rs.getInt(1)%>&email=<%=emailid%>">Book</a></td></tr>
                  <% 
